...
const RULES = [
  ...
  ["⚠️ 其他規範", [
    ...
    ["聊天室辱罵、洗版、散布不實消息", "視情節禁言;情節重大或屢勸不聽者停權。<b>把未經官方公告的猜測當成「內部消息」散布、誤導其他玩家</b>,同樣算散布不實消息。"],
...
const COMMUNITY_RULES = [
  ...
  ["🚫 禁止事項", [
    ...
    ["散布未經官方公告的「內部消息」", "更新內容、活動規則、開放時間一律以官方公告與官網為準;<b>公告前沒有任何人知道內容,客服也一樣</b>。把猜測當成消息在社群傳、造成其他玩家誤信者先禁言;刻意誤導、屢勸不聽者請離開社群。(2026-09-26 新增)"],
  ]],
  ["✅ 歡迎你這樣說", [
...
// 📢 首頁置頂公告(2026-09-26 麥哥:玩家一直問客服進度/活動內容、半夜有人把猜測當消息傳)。
//    要撤就把 HOME_NOTICE 設成 "";內容與社群/DC 貼的那則同一份(麥哥把關過)。
const HOME_NOTICE = `<div class="mcard" style="border-color:#8a6d2e;background:#241d10;margin:6px 0 14px">
<div class="ttl">📢 關於遊戲進度、更新與活動消息</div>
<div class="sub" style="line-height:1.9">
1️⃣ 所有更新內容、活動規則、開放時間，一律以<b>官方公告</b>與<b>本站</b>為準。公告發出之前，任何人說的都不是官方消息——包括「聽說」「內部人講的」「客服偷偷告訴我的」。<br>
2️⃣ <b>客服不會、也無法提前透露未公布的內容。</b>客服看到的資訊和大家一樣，問「下次更新什麼」只會得到「請等官方公告」；這不是敷衍，是沒有人可以提前知道。<br>
3️⃣ <b>請不要在社群散布未經公告的內容。</b>把自己的猜測當成消息傳，會讓其他玩家誤信、覺得「為什麼有人先知道」——再次強調：<b>沒有人先知道</b>。刻意散布假消息誤導他人，依<a href="rules.html" style="color:#f5c451">遊戲規章</a>處理。<br>
更新與活動一定會提前公告，請看<a href="changelog.html" style="color:#f5c451">版本更新</a>與<a href="event.html" style="color:#f5c451">活動介紹</a>，不需要猜、也不用問。
</div></div>`;
...
if (FEAT.codex && GD.codexBonus) {
  ...
  const rows = Object.entries(GD.codexBonus)
    ...
    .filter(r => r.m && (r.m.lv <= 49 || r.d.open) && mobZones[r.m.n] && STAT[r.d.stat]) // mobZones=玩家進得去的獵場才有的怪(同怪物頁過濾);麥哥 9/14 晚:Lv40~49 也上;9/25:`open:true` 的特別開放格也上(遺忘之島獨眼巨人/飛龍)
    .sort((a, b) => (a.m.lv - b.m.lv) || a.m.n.localeCompare(b.m.n));
  const need = lv => lv < 20 ? 50 : lv < 40 ? 200 : 500;
  const cost = (lv, d) => `${(lv * 20000).toLocaleString()} 金幣${lv >= 40 ? ` + ${(d && d.bd) || 10} 藍鑽` : ""}`; // d.bd=該格藍鑽覆寫(同 engine CodexCostOf)
  const special = rows.filter(r => r.d.open && r.m.lv >= 50);
  const esc = s => String(s).replace(/&/g, "&amp;").replace(/</g, "&lt;");
...
${special.length ? `<tr><td>特別開放：${special.map(r => esc(r.m.n)).join("、")}</td><td>500 次</td><td>怪物等級 × 2 萬金幣 <b>+ ${special[0].d.bd || 10} 藍鑽</b></td></tr>` : ""}
<tr><td>其他 Lv50 以上・世界王</td><td colspan="2">尚未開放</td></tr>
</tbody></table></div>
...
<div class="mcard"><div class="ttl">🗂 已開放各怪登錄效果(Lv1 ~ 49 + 特別開放,共 ${rows.length} 格)</div>
<div class="wrap"><table><thead><tr><th>等級</th><th>怪物</th><th>點亮</th><th>登錄費</th><th>登錄效果</th></tr></thead><tbody>
${rows.map(r => `<tr><td>${r.m.lv}</td><td>${esc(r.m.n)}${r.d.open && r.m.lv >= 50 ? " <span style='color:#f5a97f;font-size:12px'>特別開放</span>" : ""}</td><td>${need(r.m.lv)} 次</td><td>${cost(r.m.lv, r.d)}</td><td>${bonusTxt(r.d)}</td></tr>`).join("\n")}
</tbody></table></div>
...
<div class="mcard" style="margin-bottom:12px"><div class="ttl">⚔ 騎士武器精通:第二武器精通的「共享加成」怎麼算</div>
<div class="sub" style="line-height:1.9">・武器精通滿級後可選一種能力:<b>命中 +15</b>、<b>傷害 +15</b>、或<b>平衡(命中 +9、傷害 +9)</b>,手持綁定的那把武器(武器共鳴開著時同類型武器也算)才生效。<br>
・開通<b>第二武器精通</b>後,兩把武器各有一個精通槽;<b>沒拿在手上的那一把</b>(待命槽)只要<b>已滿級且已選能力</b>,它的能力值 <b>÷3(無條件捨去)</b> 會疊到你手上這把上:命中型 +5、傷害型 +5、平衡型 命中 +3、傷害 +3。<br>
・共享加成<b>跟著手上這把的能力光環走</b>:主武器的光環開著、且手持綁定武器時才一起生效;光環沒開或換拿別的武器,主加成與共享加成一起消失。<br>
・待命槽沒滿級、或還沒選能力,就沒有共享加成。切換/指定第二武器精通每次 100 萬金幣。精通分頁的「共享」欄顯示的數字就是實際生效的值。</div></div>
...
・「攻速」是<b>百分比</b>加成,其餘都是平加。<br>・<b>攻速同時加快平砍與施法</b>:平砍間隔與技能施放間隔(基礎 2 秒)都乘上同一個 (1 − 攻速%)——攻速 +20% 就是平砍快 20%、施法間隔變 1.6 秒;加速術/勇敢藥水也是同一套算法,兩者相乘。
</div></div>
${secs}`));
}
